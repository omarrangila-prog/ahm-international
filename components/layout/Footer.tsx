...
export function Footer() {
  ...
  return (
    <footer className="tooth relative bg-ink text-cream" data-zone="dark">
        ...
        <div className="grid grid-cols-12 gap-y-12 lg:gap-x-8">
          ...
          {footerNav.map((column) => (
            <div key={column.title} className="col-span-6 sm:col-span-4 lg:col-span-2">
              <p className="label mb-5 text-cream/60">{column.title}</p>
              <ul className="flex flex-col gap-1 sm:gap-1.5">
                {column.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="inline-block py-1.5 text-sm text-cream/80 transition-colors hover:text-lime"
                    >
                      {link.label}
                    </Link>
                  </li>
          ...
          <div className="col-span-12 sm:col-span-4 lg:col-span-2">
            <p className="label mb-5 text-cream/60">Contact</p>
            <ul className="flex flex-col gap-4 text-sm">
              <li>
                <p className="text-cream/60">Email</p>
                {email ? (
                  <a href={`mailto:${email}`} className="inline-block py-1.5 text-cream/80 transition-colors hover:text-lime">
                    {email}
                  </a>
                ) : (
              ...
              <li>
                ...
                {phone ? (
                  <a href={`tel:${phone.replace(/\s/g, "")}`} className="inline-block py-1.5 text-cream/80 transition-colors hover:text-lime">
                    {phone}
                  </a>
                ) : (
                  <p className="text-cream/60">{NOT_PUBLISHED}</p>
              ...
              <div className="mt-6 flex items-center gap-3">
                {linkedin && (
                  <a
                    ...
                    className="flex h-10 w-10 items-center justify-center border border-line-invert text-cream/70 transition-colors hover:border-lime hover:text-lime"
        ...
        <div className="mt-20 border-t border-line-invert pt-12 lg:mt-28">
          <Link href="/request-a-quote" className="group block">
            <MaskedHeading
              ...
              as="p"
              className="font-display text-mega text-cream transition-colors duration-500 group-hover:text-lime"
            />
          </Link>
        </div>

        <div className="mt-14 flex flex-col gap-4 border-t border-line-invert pt-7 text-xs text-cream/60 sm:flex-row sm:items-center sm:justify-between">
          <p>
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
            {[
              ...
              <Link key={link.href} href={link.href} className="inline-block py-1.5 text-cream/70 transition-colors hover:text-lime">
                {link.label}
              </Link>
            ))}
            <span className="max-w-md text-cream/70">
