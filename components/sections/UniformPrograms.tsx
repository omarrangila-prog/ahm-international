...
export function UniformPrograms() {
  ...
  return (
    <Section zone="forest" spacing="lg" tooth aria-labelledby="programs-heading">
      <div className="shell-wide relative z-10">
        <div className="max-w-4xl">
          <MaskedHeading
            ...
            />
          <p className="mt-7 max-w-2xl text-lead text-cream/70">
          ...
          <ul className="flex flex-wrap gap-2">
            {industries.map((industry) => (
              <li
                ...
                className="border border-cream/20 px-3.5 py-2 text-xs text-cream/75 transition-colors hover:border-lime hover:text-lime"
              >
                {industry.name}
              </li>
            ))}
        ...
        {/* Performance options */}
        <div className="mt-20 grid grid-cols-12 gap-y-8 border-t border-cream/15 pt-12 lg:gap-x-12">
          <div className="col-span-12 lg:col-span-4">
            <h3 className="font-display text-h2 text-cream">
              Performance is specified,
              <span className="block text-cream/60">not promised.</span>
            </h3>
          </div>
              ...
              {performanceOptions.map((option) => (
                <RevealItem key={option.name}>
                  <div className="border-l border-lime/40 pl-4">
                    <p className="font-display text-[0.9375rem] font-bold tracking-[-0.015em] text-cream">
                      {option.name}
                    </p>
                    <p className="mt-1 text-xs leading-relaxed text-cream/70">{option.note}</p>
                  </div>
                </RevealItem>
              ))}
            </RevealGroup>

            <p className="mt-9 max-w-2xl border-t border-cream/15 pt-5 text-xs text-cream/60">
